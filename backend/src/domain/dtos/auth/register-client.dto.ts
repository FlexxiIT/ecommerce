...
export class RegisterClientDto {
    ...
    static create(object: { [key: string]: any }): [string?, RegisterClientDto?] {
        ...
        if (name.length < 3 || name.length > 20) return ['The name must have at least 8 minimum caracters and 20 maximum caracters'];
        if (!surname) return ['Missing surname'];
        if (surname.length < 3 || surname.length > 20) return ['The surname must have at least 8 minimum caracters and 20 maximum caracters'];
